...
class InputExample(object):
    ...
    def __init__(self, guid, text_a, text_b=None, label=None, label2=None, label3=None,):
...
class InputFeatures(object):
    ...
    def __init__(self, input_ids, input_mask, segment_ids, label_id, label_id2, label_id3):
        self.input_ids = input_ids
        self.input_mask = input_mask
        self.segment_ids = segment_ids
        self.label_id = label_id
        self.label_id2 = label_id2
        self.label_id3 = label_id3
...
class IMDBProcessor(DataProcessor):
    ...
    def _create_examples(self, lines, set_type, data_num=None):
        ...
        for (i, line) in enumerate(lines):
            ...
            label = tokenization.convert_to_unicode(str(line[0]))
            label2 = tokenization.convert_to_unicode(str(line[1]))
            label3 = tokenization.convert_to_unicode(str(line[2]))
            ...
            """if i%1000==0:
                ...
                print("label=",label)""",
            examples.append(
                InputExample(guid=guid, text_a=text_a, text_b=None, label=label, label2=label2, label3=label3))
        return examples
...
def convert_examples_to_features(examples, label_list, max_seq_length, tokenizer, trunc_medium=-2):
    ...
    for (ex_index, example) in enumerate(tqdm(examples)):
        ...
        label_id3 = label_map[example.label3]
        """if ex_index < 5:
            logger.info("*** Example ***")
...
def main():
    ...
    all_label_ids3 = torch.tensor([f.label_id3 for f in eval_features], dtype=torch.long)
    

    eval_data = TensorDataset(all_input_ids, all_input_mask, all_segment_ids, all_label_ids, all_label_ids2, all_label_ids3)
    ...
    if args.do_train:
        ...
        all_label_ids3 = torch.tensor([f.label_id3 for f in train_features], dtype=torch.long)

        train_data = TensorDataset(all_input_ids, all_input_mask, all_segment_ids, all_label_ids, all_label_ids2, all_label_ids3)
        if args.local_rank == -1:
        ...
        for _ in trange(int(args.num_train_epochs), desc="Epoch"):
            ...
            for step, batch in enumerate(tqdm(train_dataloader, desc="Iteration")):
                ...
                input_ids, input_mask, segment_ids, label_ids, label_ids2,label_ids3 = batch
                loss, _, _, _ = model(input_ids, segment_ids, input_mask, label_ids, label_ids2,label_ids3)
                if n_gpu > 1:
            ...
            with open(os.path.join(args.output_dir, "results_ep"+str(epoch)+".txt"),"w") as f:
                for input_ids, input_mask, segment_ids, label_ids, label_ids2, label_ids3 in tqdm(eval_dataloader, desc="Evaluate"):
                    ...
                    with torch.no_grad():
                        tmp_eval_loss, logits, logits2, logits3 = model(input_ids, segment_ids, input_mask, label_ids, label_ids2, label_ids3)
                    ...
                    l = softmax(logits, axis = 1).tolist()
                    outputs = np.argmax(logits, axis=1)
                    tup = zip(outputs, l)
                    for output, logit in tup:
                        f.write("{},{}\n".format(str(output),str(logit)))


            model.eval()
            eval_loss, eval_accuracy = 0, 0
            nb_eval_steps, nb_eval_examples = 0, 0
            with open(os.path.join(args.output_dir, "results_ep2"+str(epoch)+".txt"),"w") as f:
                for input_ids, input_mask, segment_ids, label_ids, label_ids2, label_ids3 in tqdm(eval_dataloader, desc="Evaluate"):
                    input_ids = input_ids.to(device)
                    input_mask = input_mask.to(device)
                    segment_ids = segment_ids.to(device)
                    ...
                    label_ids2 = label_ids2.to('cpu').numpy()
                    label_ids3 = label_ids3.to('cpu').numpy()
                    l = softmax(logits2, axis = 1).tolist()
                    outputs2 = np.argmax(logits2, axis=1)
                    tup = zip(outputs2, l)
                    for output, logit in tup:
                        f.write("{},{}\n".format(str(output),str(logit)))
                    #tmp_eval_accuracy=np.sum(outputs2 == label_ids2)
           ...
           # eval_accuracy = eval_accuracy / nb_eval_examples
            ...
            eval_loss, eval_accuracy = 0, 0
            nb_eval_steps, nb_eval_examples = 0, 0
            with open(os.path.join(args.output_dir, "results_ep3"+str(epoch)+".txt"),"w") as f:
                for input_ids, input_mask, segment_ids, label_ids, label_ids2, label_ids3 in tqdm(eval_dataloader, desc="Evaluate"):
                    input_ids = input_ids.to(device)
                    input_mask = input_mask.to(device)
                    segment_ids = segment_ids.to(device)
                    ...
                    logit2 = logits2.detach().cpu().numpy()
                    logit3 = logits3.detach().cpu().numpy()
                    label_ids = label_ids.to('cpu').numpy()
                    label_ids2 = label_ids2.to('cpu').numpy()
                    label_ids3 = label_ids3.to('cpu').numpy()
                    l = softmax(logit3, axis = 1).tolist()
                    outputs3 = np.argmax(logit3, axis=1)
                    tup = zip(outputs3, l)
                    for output, logit in tup:
                        f.write("{},{}\n".format(str(output),str(logit)))
                    #tmp_eval_accuracy=np.sum(outputs3 == label_ids3)
